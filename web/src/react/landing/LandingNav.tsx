...
const LOCALE_NAMES: Record<PublicLocale, string> = {
  ...
  return (
    <nav className="nav" aria-label={t('landing.navPrimary')}>
      <div className="nav-inner">
        <button className="nav-brand" onClick={() => go('top')} aria-label={t('landing.navBackToTop')}>
        ...
        <div className={`nav-links${open ? ' open' : ''}`} id="nav-links">
          {LINKS.map((link) => <button onClick={() => go(link.id)} key={link.id}>{t(link.key)}</button>)}
        </div>
        ...
        <div className="nav-actions">
          ...
          <button className="btn btn-primary btn-sm" onClick={enterApp}>{t('landing.navOpenEditor')}</button>
          <button
