...
import { snapshotState, stateValue } from './state-value';
import {
...
import type { ModelSnapshot, SnapshotKind } from './history';
...
function createModelStore() {
  ...
  let model = stateValue<StructureModel>({
  ...
  let lastKinematicResult = stateValue<KinematicResult | null>(null);
  let modelVersion = stateValue(0);
  ...
  return {
    ...
    reinforcementTransaction(
      ...
      for (const [elemId, r] of pending) {
        ...
        if (!elem) continue;
        const plain = snapshotState(elem) as Element;
        plain.reinforcement = r
          ? (JSON.parse(JSON.stringify(snapshotState(r))) as ProvidedReinforcement)
          : undefined;
    ...
    snapshot(): ModelSnapshot {
      ...
      const snap = snapshotState(model);
      const snapId = snapshotState(nextId);
      const result: ModelSnapshot = {
        ...
        constraints: snap.constraints as unknown as ModelSnapshot['constraints'],
