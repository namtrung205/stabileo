...
import type { AnalysisResults, InfluenceLineResult, Section, Material } from './model';
...
function createResultsStore() {
  let results = stateValue<AnalysisResults | null>(null);
  let diagramType = stateValue<DiagramType>('none');
  /** Remembers last user-visible diagram so live-calc can restore it after clear() */
  let _lastDiagramType: DiagramType = 'none';
  let deformedScale = stateValue<number>(1); // Scale factor for deformed shape (applied directly to displacements)
  let diagramScale = stateValue<number>(1); // Multiplier for M/V/N diagram size (1 = default 60px height)
  let animateDeformed = stateValue<boolean>(false);
  /**
   ...
   */
  let colourScale = stateValue<{ max: number; unit: string; source: string } | null>(null);
  let colorMapKind = stateValue<'moment' | 'shear' | 'axial' | 'momentY' | 'momentZ' | 'shearY' | 'shearZ' | 'torsion' | 'stressRatio' | 'vonMises' | 'sigmaMax' | 'tauMax' | 'shellVonMises' | 'shellBending'>('moment');
  // Which shell quantity the shell contour paints (selectable in PRO results).
  let shellContourComponent = stateValue<import('../engine/shell-stress').ShellContourComponent>('vonMises');
  let showDiagramValues = stateValue<boolean>(true);
  ...
  let drawPositiveTowardLocalAxes = stateValue<boolean>(false);
  let animSpeed = stateValue<number>(1.0); // animation speed multiplier (0.25 - 3x)

  // Combination results
  let singleResults = stateValue<AnalysisResults | null>(null); // base solve (all loads, no combination factors)
  let perCase = stateValue<Map<number, AnalysisResults>>(new Map());
  let perCombo = stateValue<Map<number, AnalysisResults>>(new Map());
  let envelope = stateValue<FullEnvelope | null>(null);
  let activeView = stateValue<ResultsView>('single');
  let activeComboId = stateValue<number | null>(null);
  let activeCaseId = stateValue<number | null>(null); // individual load case selection

  // Influence line
  let influenceLine = stateValue<InfluenceLineResult | null>(null);
  let ilAnimating = stateValue<boolean>(false);
  let ilAnimProgress = stateValue<number>(0); // 0..1 progress along structure
  let ilAnimSpeed = stateValue<number>(1.0);

  // Combination dirty flag (set when combos/cases are modified after solving)
  let combinationsDirty = stateValue<boolean>(false);

  // Overlay comparison
  let overlayResults = stateValue<AnalysisResults | null>(null);
  let overlayResults3D = stateValue<AnalysisResults3D | null>(null);
  let overlayLabel = stateValue<string>('');

  // Advanced analysis results
  let movingLoadEnvelope = stateValue<MovingLoadEnvelope | null>(null);
  let activeMovingLoadPosition = stateValue<number>(0);
  let pdeltaResult = stateValue<PDeltaResult | null>(null);
  let modalResult = stateValue<ModalResult | null>(null);
  let activeModeIndex = stateValue<number>(0);
  let bucklingResult = stateValue<BucklingResult | null>(null);
  let activeBucklingMode = stateValue<number>(0);
  let plasticResult = stateValue<PlasticResult | null>(null);
  let plasticStep = stateValue<number>(0);
  let spectralResult = stateValue<SpectralResult | null>(null);
  // 3D advanced analysis results
  let pdeltaResult3D = stateValue<PDeltaResult3D | null>(null);
  let modalResult3D = stateValue<ModalResult3D | null>(null);
  let bucklingResult3D = stateValue<BucklingResult3D | null>(null);
  let spectralResult3D = stateValue<SpectralResult3D | null>(null);
  let showReactions = stateValue<boolean>(false);
  let showConstraintForces = stateValue<boolean>(false);
  let movingLoadShowEnvelope = stateValue<boolean>(false);

  // Moving load progress tracking
  let movingLoadRunning = stateValue<boolean>(false);
  let movingLoadProgress = stateValue<{ current: number; total: number } | null>(null);
  let movingLoadAbortController: AbortController | null = null;

  // Section stress query (click on element to analyze)
  let stressQuery = stateValue<{
    elementId: number;
  ...
  // 3D analysis results
  let results3D = stateValue<AnalysisResults3D | null>(null);
  let singleResults3D = stateValue<AnalysisResults3D | null>(null);
  let perCase3D = stateValue<Map<number, AnalysisResults3D>>(new Map());
  let perCombo3D = stateValue<Map<number, AnalysisResults3D>>(new Map());
  let envelope3D = stateValue<FullEnvelope3D | null>(null);

  // Governing-case provenance (which combo governs each element per force component)
  let governing2D = stateValue<Map<number, GoverningPerElement>>(new Map());
  let governing3D = stateValue<Map<number, GoverningPerElement3D>>(new Map());

  // Diagnostics (2D + 3D parallel)
  let diagnostics2D = stateValue<SolverDiagnostic[]>([]);
  let constraintForces2D = stateValue<ConstraintForce[]>([]);
  let diagnostics3DArr = stateValue<SolverDiagnostic[]>([]);
  let constraintForces3DArr = stateValue<ConstraintForce[]>([]);

  // Solve timings
  let solveTimings2D = stateValue<SolveTimings | null>(null);
  let solveTimings3D = stateValue<SolveTimings | null>(null);
