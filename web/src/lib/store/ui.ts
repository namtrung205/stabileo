...
function createUIStore() {
  ...
  let currentTool = stateValue<Tool>('pan');
  let supportType = stateValue<SupportTool>('pinned');
  let loadType = stateValue<LoadTool>('nodal');
  let nodalLoadDir = stateValue<NodalLoadDir>('fz'); // direction for nodal load placement
  let loadValue = stateValue<number>(-10); // kN, negative = downward
  let loadValueJ = stateValue<number>(-10); // kN/m at node J (for trapezoidal)

  // Spring stiffnesses for spring support tool
  let springKx = stateValue<number>(1000); // kN/m
  let springKy = stateValue<number>(1000); // kN/m
  let springKz = stateValue<number>(0);    // kN·m/rad

  // Prescribed displacements for support tool
  let supportDx = stateValue<number>(0); // m
  let supportDy = stateValue<number>(0); // m
  let supportDrz = stateValue<number>(0); // rad

  // Thermal load defaults
  let thermalDT = stateValue<number>(30);  // °C uniform
  let thermalDTg = stateValue<number>(0);  // °C gradient

  // Load angle & coordinate system
  let loadAngle = stateValue<number>(0);       // degrees, default 0
  let loadIsGlobal = stateValue<boolean>(false); // false = local/perpendicular (default), true = displayed global Z

  // Roller support configuration
  let supportIsGlobal = stateValue<boolean>(true); // true = ejes globales (default)
  let supportDirection = stateValue<'x' | 'y'>('x'); // x/y en global, i/j en local
  let supportAngle = stateValue<number>(0); // ángulo custom en grados

  let gridSize = stateValue<number>(1); // meters
  let snapToGrid = stateValue<boolean>(true);
  let showGrid = stateValue<boolean>(true);
  ...
  // point).
  let autoSplitOnNodePlace = stateValue<boolean>(true);

  let zoom = stateValue<number>(50); // pixels per meter
  let panX = stateValue<number>(400);
  let panY = stateValue<number>(300);

  let selectMode = stateValue<SelectMode>('elements');
   ...
   */
  let multiKindSelect = stateValue<boolean>(false);

  /** Kinds a click or a drag picks up. Never empty: one is always active. */
  let selectKinds = stateValue<Set<SelectMode>>(new Set(['elements']));
  ...
  let selectedNodes = stateValue<Set<number>>(new Set());
  let selectedElements = stateValue<Set<number>>(new Set());
  ...
  let elementSelectionManual = stateValue<boolean>(false);
  let selectedLoads = stateValue<Set<number>>(new Set());
  let selectedSupports = stateValue<Set<number>>(new Set());
  ...
  let selectedShells = stateValue<Set<string>>(new Set());
  ...
  let shellNodePick = stateValue<{ active: boolean; target: 'plate' | 'quad' | 'mesh' | null; picked: number[]; capacity: number }>(
    { active: false, target: null, picked: [], capacity: 0 },
  );

  let mouseX = stateValue<number>(0);
  let mouseY = stateValue<number>(0);
  let worldX = stateValue<number>(0);
  let worldY = stateValue<number>(0);

  // Inline editing state
  let editingNodeId = stateValue<number | null>(null);
  let editingElementId = stateValue<number | null>(null);
  let editScreenPos = stateValue<{ x: number; y: number }>({ x: 0, y: 0 });

  // Data table
  let showDataTable = stateValue<boolean>(true);

  // Material/Section editing
  let editingMaterialId = stateValue<number | null>(null);
  let editingSectionId = stateValue<number | null>(null);

  // Visualization toggles
  let showNodeLabels = stateValue<boolean>(true);
  /**
   ...
   */
  let showColourScale = stateValue<boolean>(true);
  let showElementLabels = stateValue<boolean>(false);
  let showLengths = stateValue<boolean>(false);
  let elementColorMode = stateValue<ElementColorMode>('uniform');
  let showLoads = stateValue<boolean>(true);
  let hideLoadsWithDiagram = stateValue<boolean>(true);

  // Result selector visibility
  let showPrimarySelector = stateValue<boolean>(true);
  let showSecondarySelector = stateValue<boolean>(true);

  // 3D rendering mode — per-mode
  let renderMode3D_basic = stateValue<'wireframe' | 'solid' | 'sections'>('wireframe');
  let renderMode3D_pro = stateValue<'wireframe' | 'solid' | 'sections'>('wireframe');
  ...
  let smoothOrbit3D = stateValue<boolean>(false);

  // 3D moment visualization style — per-mode
  let momentStyle3D_basic = stateValue<'double-arrow' | 'curved'>('curved');
  let momentStyle3D_pro = stateValue<'double-arrow' | 'curved'>('curved');

  // 3D camera mode
  let cameraMode3D = stateValue<'perspective' | 'orthographic'>('perspective');

  // 3D clipping plane
  let clippingEnabled = stateValue<boolean>(false);
  let clippingAxis = stateValue<'x' | 'y' | 'z'>(VERTICAL_AXIS);
  let clippingPosition = stateValue<number>(0);

  // 3D working plane and snap
  let workingPlane = stateValue<'XZ' | 'XY' | 'YZ'>(DEFAULT_WORKING_PLANE);

  // Duplicate along axis
  let duplicateAxis = stateValue<'x' | 'y' | 'z'>('z');
  let duplicateDistance = stateValue<number>(3);

  // Clipboard
  let clipboard = stateValue<ClipboardData | null>(null);

  // Self-weight — per-mode so switching modes doesn't affect each other
  let selfWeightBasico = stateValue(false);
  let selfWeightEducativo = stateValue(false);
  let selfWeightPro = stateValue(true);

  // Element creation type
  let elementCreateType = stateValue<'frame' | 'truss'>('frame');
  let elementMode = stateValue<ElementMode>('create');
  let nodeMode = stateValue<NodeMode>('create');
  let jointType = stateValue<JointType>('hinge');
  let jointAxis = stateValue<JointAxisMode>('global');
  // Basic 3D joint placement mask [dx,dy,dz,θx,θy,θz] applied on element click.
  let jointDof3d = stateValue<boolean[]>([false, false, false, false, false, false]);
  // Despiece / free-body view: which interaction vectors to show.
  let despieceVectorMode = stateValue<'all' | 'members' | 'nodes'>('all');
  let despieceBasis = stateValue<'local' | 'global'>('local');
  let despieceVectorSize = stateValue<number>(1);   // arrow/glyph length multiplier
  let despieceLabelSize = stateValue<number>(1);     // label font multiplier
  ...
  let despieceCombineVectors = stateValue<boolean>(false);
  ...
  let despieceLoadMode = stateValue<'off' | 'resultant' | 'all'>('off');
  // Click inspection target while Despiece is active (null = none).
  let despieceInspect = stateValue<{ type: 'node' | 'member'; id: number } | null>(null);

  // Active load case for load tool
  let activeLoadCaseId = stateValue<number>(1);

  // Influence line quantity
  let ilQuantity = stateValue<ILQuantity>('Rz');

  // Help overlay
  let showHelp = stateValue<boolean>(false);

  // Embed mode (hides header, sidebars, footer when loaded via #embed= URL)
  let embedMode = stateValue<boolean>(false);
  ...
  let showFloatingTools = stateValue<boolean>(savedFloatingTools !== 'false'); // default true
  ...
  let floatingToolsRows = stateValue<number>(1);
  ...
  let showTooltips = stateValue<boolean>(savedTooltips !== 'false'); // default true
  ...
  let showHelpPanel = stateValue<boolean>(savedHelpPanel === 'true'); // default false

  // Unit system — persisted in localStorage
  const savedUnitSystem = hasLocalStorage() ? localStorage.getItem('stabileo-unitSystem') : null;
  let unitSystem = stateValue<UnitSystem>((savedUnitSystem === 'Imperial' ? 'Imperial' : 'SI') as UnitSystem);

  // What-If exploration mode (not persisted — temporary)
  let showWhatIf = stateValue<boolean>(false);

  // Kinematic analysis panel (not persisted — temporary)
  let showKinematicPanel = stateValue<boolean>(false);

  // Mobile responsive
  let windowWidth = stateValue(initialWindowWidth);
  let leftDrawerOpen = stateValue(false);
  let rightDrawerOpen = stateValue(false);
  let mobileResultsPanelOpen = stateValue(false);

  // Desktop sidebar toggles
  let rightSidebarOpen = stateValue(false);
  let leftSidebarOpen = stateValue(true);
  let proPanelVisible = stateValue(true);
  let proPanelWidth = stateValue(540);

  // PRO panel active tab (shared so App.svelte can render the nav strip)
  let proActiveTab = stateValue('nodes');

  // AI drawer
  let aiDrawerOpen = stateValue(false);

  // Context menu
  let contextMenu = stateValue<{ x: number; y: number; nodeId?: number; elementId?: number } | null>(null);
  ...
  let toasts = stateValue<Array<{ id: number; message: string; type: 'success' | 'error' | 'info'; actionId?: string }>>([]);
  let toastCounter = 0;

  // Live calculation
  let liveCalc = stateValue(hasLocalStorage() && localStorage.getItem('liveCalc') === 'true');
  let liveCalcError = stateValue<string | null>(null);

  // Basic analysis dimension. Legacy PRO/EDU values are accepted by the
  // setter only so older saved files can be opened and normalized.
  let analysisMode = stateValue<'2d' | '3d' | 'pro' | 'edu'>('2d');
  ...
  let drawPlane2D = stateValue<'xy' | 'xz' | 'yz'>('xy');
   ...
   * buttons.
   */
  let switchTo2DPrompt = stateValue(false);
  ...
  let simplified2DMode = stateValue(false);
  /**
   ...
   */
  let simplified2DStats = stateValue<{
  ...
  let viewportPresentation3D = stateValue<ViewportPresentation3D>('native3d');
  ...
  let nodalLoadDir3D = stateValue<NodalLoadDir3D>('fy');
  let loadValueZ = stateValue<number>(0); // For Fz or qZI components
  let loadValueZJ = stateValue<number>(0); // For qZJ components (3D distributed)

  // 3D support type
  let supportType3D = stateValue<SupportTool3D>('pinned3d');

  // 3D spring rotational stiffnesses
  let springKrx = stateValue<number>(0); // kN·m/rad
  let springKry = stateValue<number>(0); // kN·m/rad
  let springKrz = stateValue<number>(0); // kN·m/rad

  // 3D support prescribed displacements (additional DOFs)
  let supportDz = stateValue<number>(0);  // m
  let supportDrx = stateValue<number>(0); // rad
  let supportDry = stateValue<number>(0); // rad

  // 3D per-DOF support creation state
  let supportFrame3D = stateValue<'global' | 'local'>('global');
  let sup3dTx = stateValue(true);
  let sup3dTy = stateValue(true);
  let sup3dTz = stateValue(true);
  let sup3dRx = stateValue(false);
  let sup3dRy = stateValue(false);
  let sup3dRz = stateValue(false);
  // Spring stiffnesses per DOF (used when DOF unchecked and user wants spring)
  let sup3dKx = stateValue(0);
  let sup3dKy = stateValue(0);
  let sup3dKz = stateValue(0);
  let sup3dKrx = stateValue(0);
  let sup3dKry = stateValue(0);
  let sup3dKrz = stateValue(0);

  // Node creation Y level for 3D (ground plane height)
  let nodeCreateZ = stateValue<number>(0);

  // Measurement tool
  let measureMode = stateValue<boolean>(false);
  let measurePoints = stateValue<Array<{x: number; y: number; z: number}>>([]);

  // Show axes (2D)
  let showAxes = stateValue<boolean>(true);

  // Independent 3D visualization config — Basic 3D mode
  let showGrid3D_basic = stateValue<boolean>(true);
  let snapToGrid3D_basic = stateValue<boolean>(true);
  let gridSize3D_basic = stateValue<number>(1);
  let gridExtent3D_basic = stateValue<number>(50);
  let showNodeLabels3D_basic = stateValue<boolean>(true);
  let showElementLabels3D_basic = stateValue<boolean>(false);
  let showLengths3D_basic = stateValue<boolean>(false);
  let showLoads3D_basic = stateValue<boolean>(true);
  let visibleLoadCases3D_basic = stateValue<number[] | null>(null);
  let showAxes3D_basic = stateValue<boolean>(true);
  let localAxesMode3D_basic = stateValue<LocalAxesDisplayMode>('selected');
  let shellAxesMode3D_basic = stateValue<LocalAxesDisplayMode>('selected');

  // Independent 3D visualization config — PRO mode
  let showGrid3D_pro = stateValue<boolean>(true);
  let snapToGrid3D_pro = stateValue<boolean>(true);
  let gridSize3D_pro = stateValue<number>(1);
  let gridExtent3D_pro = stateValue<number>(50);
  let showNodeLabels3D_pro = stateValue<boolean>(true);
  let showElementLabels3D_pro = stateValue<boolean>(false);
  let showLengths3D_pro = stateValue<boolean>(false);
  let showLoads3D_pro = stateValue<boolean>(true);
  let visibleLoadCases3D_pro = stateValue<number[] | null>(null);
  let showAxes3D_pro = stateValue<boolean>(true);
  let localAxesMode3D_pro = stateValue<LocalAxesDisplayMode>('selected');
  let shellAxesMode3D_pro = stateValue<LocalAxesDisplayMode>('selected');

  // 3D axis convention: terna derecha (right-hand, default) or terna izquierda (left-hand)
  let axisConvention3D = stateValue<'rightHand' | 'leftHand'>('rightHand');

  // 3D camera state (synced from Viewport3D via saveCameraState, restored on tab switch)
  let cameraPosition3D = stateValue<{ x: number; y: number; z: number }>({ x: 10, y: 8, z: 10 });
  let cameraTarget3D = stateValue<{ x: number; y: number; z: number }>({ x: 0, y: 0, z: 0 });

  // Pending auto-solve from URL sharing (stores the diagramType to restore after solve)
  let pendingSolveFromURL = stateValue<string | null>(null);

  // Continuous rendering override (forces requestAnimationFrame loop like old behavior)
  let continuousRendering = stateValue<boolean>(false);
  ...
  return {
    ...
    set analysisMode(v: '2d' | '3d' | 'pro' | 'edu') {
      analysisMode = v === 'pro' ? '3d' : v === 'edu' ? '2d' : v;
      ...
      if (analysisMode === '3d' && _isModelFlat2D?.() === true) {
    ...
    /** The product now has a single Basic editor shell. */
    get appMode(): 'basico' { return 'basico'; },
