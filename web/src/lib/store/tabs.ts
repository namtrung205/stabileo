...
import type { Tool, SelectMode, ElementColorMode } from './ui';
...
export interface TabState {
  ...
  // ('structural' vs 'reinforcement' — see history.ts). Optional so any
...
function createTabManager() {
  let tabs = stateValue<TabState[]>([]);
  let activeTabId = stateValue<string | null>(null);
  ...
  let _tabSwitching = stateValue(false);
