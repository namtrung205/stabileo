...
function createVerificationStore() {
  ...
  let concreteVerifs = stateValue<ElementVerification[]>([]);
  let steelVerifs = stateValue<SteelVerification[]>([]);
  let concreteMap = stateValue<Map<number, ElementVerification>>(new Map());
  let steelMap = stateValue<Map<number, SteelVerification>>(new Map());

  // ── Unified code-check baseline ──
  let designResults = stateValue<MemberDesignResult[]>([]);
  let designMap = stateValue<Map<number, MemberDesignResult>>(new Map());
  let designSummary = stateValue<DesignCheckSummary | null>(null);

  // ── Revisions ──
  let analysisRevision = stateValue(1);
  let demandRevision = stateValue(0);
  let baselineRevision = stateValue(0);
  ...
  let solveGeneration = stateValue(0);

  // ── Retained demand data (independent of presentation) ──
  let contexts = stateValue<Map<number, MemberContext>>(new Map());
  let orientationIssues = stateValue<OrientationIssue[]>([]);

  // ── Design outcomes (VERIFIED / SECTION_INADEQUATE / …) ──
  let runSummary = stateValue<DesignRunSummary | null>(null);
  ...
  /** Bumped whenever the cache is invalidated, so derivedValue consumers recompute. */
  let providedRevision = stateValue(0);
